#!/usr/bin/env bash
# Sequential PERSONA-MODE low-rank LoRA sweep: acoustic_converter-only, ranks
# 1/2/4, alpha 16, batch 4, lr 1e-4, on the ASI-persona subset
# crosspair_hindi_latent_400_asi (one LoRA per accent x gender persona; all
# eval sources convert to ASI). See CHANGES.md "Target-conditioned low-rank
# LoRA sweep (2026-07-10)".
#
...
accent="crosspair_hindi_latent_400_asi"
# Persona-mode eval: ALL 20 reserved sources -> ASI (cross-gender clb->ASI
# included -- any user can be assigned this persona at serving time).
...
for r in $ranks; do
  name="finetune_crosspair_hindi_latent_400_asi_lora_acoustic_r${r}_alpha16"
...
for r in $ranks; do
  echo "  exp/finetune_crosspair_hindi_latent_400_asi_lora_acoustic_r${r}_alpha16/eval_compare/summary.csv"
done
