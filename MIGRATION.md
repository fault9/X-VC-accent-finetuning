...
| `scripts.phone_supervision` | `xvc.data.phone_supervision` | old script path is a compatibility re-export |
...
Repository-integrity correction: the original refactor's unanchored `data/`
ignore pattern also matched `xvc/data/`, leaving the documented schemas and
validation engine out of Git. The rule is now root-anchored (`/data/`), and
the package, tests, and compatibility wrappers are committed together.
