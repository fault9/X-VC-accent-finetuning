...
def merge_lora_weights(model: nn.Module) -> int:
    """Fold every adapter into its base ``weight`` (idempotent).

    After merging, each ``LoRALinear.forward`` is a plain linear — zero adapter
    overhead. ``unmerge()`` on each layer restores the exact base weights (the
    delta is subtracted back), so merge/unmerge is a lossless round trip up to
    floating-point addition. Returns the number of merged layers.
    """
    return merge_all_lora(model)


def load_lora_state_dict(model: nn.Module, state_dict: dict) -> List[str]:
    """Load ONLY the adapter tensors (``lora_A``/``lora_B``) from `state_dict`.

    Accepts either a bare generator state_dict or the two historical on-disk
    layouts: ``{"generator": {...}}`` and EMA dumps whose keys carry an
    ``ema_model.`` prefix. Base weights in `state_dict` are ignored, so this
    can apply a trained adapter onto an independently warm-started base.

    Raises ``KeyError`` if the state_dict contains adapter tensors that have no
    matching ``LoRALinear`` in `model` (topology mismatch), and ``ValueError``
    if it contains no adapter tensors at all. Returns the loaded key names.
    """
    if "generator" in state_dict and isinstance(state_dict["generator"], dict):
        state_dict = state_dict["generator"]
    if any(k.startswith("ema_model.") for k in state_dict):
        state_dict = {
            (k[len("ema_model."):] if k.startswith("ema_model.") else k): v
            for k, v in state_dict.items()
        }

    lora_sd = {k: v for k, v in state_dict.items() if is_lora_param_name(k)}
    if not lora_sd:
        raise ValueError(
            "state_dict contains no lora_A/lora_B tensors -- is this a LoRA "
            "checkpoint? (A merged export has the adapters folded into the "
            "base weights and loads via the stock path instead.)"
        )
    model_keys = {k for k, _ in model.named_parameters() if is_lora_param_name(k)}
    unmatched = sorted(set(lora_sd) - model_keys)
    if unmatched:
        raise KeyError(
            f"{len(unmatched)} adapter tensor(s) have no matching LoRALinear in "
            f"the model (inject LoRA with the SAME config the checkpoint was "
            f"trained with, before loading): {unmatched[:5]}"
        )
    missing, unexpected = model.load_state_dict(lora_sd, strict=False)
    assert not unexpected, unexpected
    return sorted(lora_sd.keys())
